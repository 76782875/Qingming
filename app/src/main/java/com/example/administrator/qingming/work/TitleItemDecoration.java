...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private int mTitleHeight;//高
    private static  int mTitleFontSize;//字体大小
    private static int COLOR_TITLE_BG = Color.parseColor("#FFDFDFDF");
    private static int COLOR_TITLE_FONT = Color.parseColor("#FF000000");

    public TitleItemDecoration(Context context, List<WorkLogModel> list){
        this.list = list;
        mpaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    ...
                    if (null != list.get(position).getTag() && !list.get(position).getTag().equals(list.get(position - 1).getTag())) {
...
/*
        ...
        mpaint.getTextBounds(list.get(position).getTag(), 0, list.get(position).getTag().length(), mBounds);
        c.drawText(list.get(position).getTag(), child.getPaddingLeft(), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mpaint);
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        String tag = list.get(pos).getTag();
        ...
        c.drawRect(parent.getPaddingLeft(), parent.getPaddingTop(), parent.getRight() -
                parent.getPaddingRight(), parent.getPaddingTop() + mTitleHeight, mpaint);
        mpaint.setColor(COLOR_TITLE_FONT);
